...
@onready var number_of_fires_currently = get_parent().get_tree().get_nodes_in_group("fires").filter(func(fire): return fire.visible == true).size()
...
func _physics_process(delta: float) -> void:
	if get_child_count() != number_of_fires_currently:
		if number_of_fires_currently < get_child_count():
			if get_child(0):
				get_child(0).queue_free()
		print(number_of_fires_currently)
	elif number_of_fires_currently > get_child_count():
		pass
	...
	number_of_fires_currently = get_parent().get_tree().get_nodes_in_group("fires").filter(func(fire): return fire.visible == true).size()
